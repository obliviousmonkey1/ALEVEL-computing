...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            ### get all legal moves ###
            self.__getAllLegalMoves(Choice, StartSquareReference)
            ###############################
    ...
    def __getAllLegalMoves(self, choice, StartSquareReference):
        ...
        for r in range(self._NoOfRows):
            for c in range(self._NoOfColumns):
                ...
                squareIsValid = self.__CheckSquareIsValid(finishSquareRef, False)
                if squareIsValid:
                    legal_move = self._CurrentPlayer.CheckPlayerMove(choice, StartSquareReference, finishSquareRef)
                    if legal_move:
                        possibleMoves.append(finishSquareRef)
